...
我们写的代码会接触到好几个环境：开发环境、测试环境以及生产环境，如果开发人员在自己电脑编写了一套程序，然后交给测试人员测试的时候需要打成war包发送到测试人员机器，测试人员测试完成后在打成war包发送到生产环境，在程序运行过程中，可能存在一个软件跨环境迁移问题，开发环境为jdk8，然后测试环境是jdk7，运行可能出错，有了docker之后，我们可以直接把环境和程序一起打包，避免了不同环境下“水土不服”的因素。
...
![架构](https://docs.docker.com/get-started/images/docker-architecture.webp)

## 4、基本概念

### （1）docker Daemon

守护进程是Docker架构中的核心部分，负责实际的容器管理工作，包括构建（build）、运行（run）和分发（distribute）Docker容器。这些操作是资源密集型的，因为容器需要在隔离的环境中运行，同时又要保持轻量级和高效。守护进程运行在主机系统上，通常是以后台服务的形式。

### （2）docker Client

客户端是用户与Docker守护进程交互的界面。用户可以通过命令行工具（CLI）或其他客户端工具发出命令，例如启动一个新容器或获取正在运行的容器列表。这些命令通过客户端发送给守护进程，守护进程执行这些命令并返回结果。
...
下面比较了 **Docker** 和传统虚拟化方式的不同之处。传统虚拟机技术是虚拟出一套硬件后，在其上运行一个完整操作系统，在该系统上再运行所需应用进程；而容器内的应用进程直接运行于宿主的内核，容器内没有自己的内核，而且也没有进行硬件虚拟。因此容器要比传统虚拟机更为轻便。
